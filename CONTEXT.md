...
**config**:
The INI file (`wraith.ini`), loaded at startup into a `Config` held behind a `OnceLock`. Holds hotkey combos, panic key, and lock-on-start flag. All 6 fields are individually mutable (`AtomicU32`/`AtomicBool`, see [ADR 0007](docs/adr/0007-mutable-config-atomics-for-settings-dialog.md)) so the settings dialog can change them without a restart and persist the change to `wraith.ini`. Combo/panic-key changes take effect immediately (the hook path re-reads them live); a `lock_on_start` change is persisted the same way but only takes effect on the next app start, since it's read exactly once, at startup. Its file location is resolved dynamically (`ini_path()`): a `wraith.ini` already sitting next to the exe wins (portable mode), otherwise it resolves to `%LOCALAPPDATA%\Wraith\wraith.ini` — the exe typically lives in Program Files once installed, which a non-elevated process can't write to. See [ADR 0009](docs/adr/0009-appdata-fallback-for-config-location.md).
